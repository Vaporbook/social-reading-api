...
if(!ubr.itemStem.match(/\/$/)){ubr.itemStem+='/';}},handleParagraphSelect:function(){Debug.log('navCtl: para selected');ubr.paragraphPointer=ubr.selectedParagraph;ubr.currentHref=ubr.currentFile+'#!'+ubr.paragraphPointer+'p:0';ubr.navCtl.setUriFrag();var dims=dojo.coords(ubr.bookSelection);var fburl='http://www.facebook.com/plugins/like.php?href='+escape(document.location.href)+'&amp;layout=standard&amp;show_faces=true&amp;width=450&amp;action=like&amp;colorscheme=light';var fbstyle={position:'absolute',top:(dims.y+dims.h-8)+'px',right:'30%',height:'30px',width:'350px',padding:'7px 3px 4px 7px',backgroundColor:'#eee',border:'2px solid #999',MozBorderRadius:'8px',WebkitBorderRadius:'8px',overflow:'hidden',display:'none',zIndex:'100000'};var fbclosestyle={position:'absolute',top:(dims.y+dims.h-2)+'px',right:'30%',border:'none',height:'12px',width:'20px',fontSize:'10px',fontFamily:'sans-serif',overflow:'hidden',cursor:'pointer',display:'none',zIndex:'100001'};if(ubr.fbLikeFrame){dojo.attr(ubr.fbLikeFrame,'src',fburl);dojo.style(ubr.fbLikeFrame,fbstyle);dojo.style(ubr.fbclose,fbclosestyle);}else{ubr.fbLikeFrame=dojo.create("iframe",{src:fburl,scrolling:'no',frameborder:'0',allowTransparency:'false',style:fbstyle},dojo.body());ubr.fbclose=dojo.create("span",{style:fbclosestyle},dojo.body());ubr.fbclose.innerHTML='x';dojo.connect(ubr.fbclose,'onclick',function(){ubr.navCtl.hideFb();});}
setTimeout(function(){dojo.style(ubr.fbLikeFrame,'display','block');dojo.style(ubr.fbclose,'display','block');},300);},handleParagraphDeSelect:function(){Debug.log('navCtl: para deselect');if(ubr.fbLikeFrame){ubr.navCtl.hideFb();}
...
{var fleft='<img src="/ubrx/rsclient/images/reader-title-flourish-left.gif" />&nbsp;';var fright='&nbsp;<img src="/ubrx/rsclient/images/reader-title-flourish-right.gif" />';dojo.byId('titleText').innerHTML=fleft+ubr.bookTitle;dojo.byId('authorText').innerHTML=ubr.bookAuthor+fright;},extendFeednav:function()
...
STAGE.feednav.innerHTML=['<div class="atomfeed"> <div class="ubrheader" style="text-align:left"><a title="Close the feed navigator" href="#" onclick="ubr.toggleCat();return false;"><img src="/ubrx/rsclient/images/reader-button-blind-up.png" alt="Close this" style="float:right" /></a>',logo,' <span class="feed-heading-lg">Catalogs</span><div class="feedtitle"><div class="backlink"><a title="Back to previously loaded feed" href="#" onclick="ubr.backAtom();return false;"><img src="/ubrx/rsclient/images/catalogicon-back.png" alt="Back to previous feed" /></a></div><div class="atom-cat-title">',title,'</div></div></div><div class="atomfeedlist"></div></div>'].join('');dojo.query(".atomfeed .atomfeedlist")[0].innerHTML=ubr.feedView.atomToHTML(obj);if(ubr.atomHistory.length==0){dojo.query(".atomfeed .backlink").style('visibility','hidden');}else{dojo.query(".atomfeed .backlink").style('visibility','visible');}}}
...
UbrMuc=function(id)
{dojo.subscribe('ubr/plugins/'+id+'/nodeready',this,function(){if(!arguments[0])throw'Cannot initialize chat plugin';arguments[0].innerHTML='<div id="profileHeader"> <div id="rangeSelect"> <label for="range">RANGE</label><br/> <select id="range"> <option value="0">WHOLE BOOK</option> <option value="1">INSIDE CHAPTER</option> </select> </div> <div id="profileImg"><img src="/ubrx/rsclient/images/profile-defaultimg-sm.jpg" /></div> </div> <!--second row --> <div id="talkflourish"></div> <div id="talktics"></div> <!-- third row --> <div id="connectionUI"> <div id="connectionState"></div> <div id="connectionMsgs">Not connected.</div> <div id="showRoster"></div> <div id="rosterCount">0</div> <div id="roomRoster"></div> </div> <!-- fourth row --> <div id="chatLogWrapper"> <div id="chatLog">**You must be logged in and reading a book to chat.<a href="#login" onclick="ubr.authUser();return false">Log in now</a></div> <textarea id="chatInput"></textarea> <div id="reader-sayit"><img id="reader-sayit-img" src="/ubrx/rsclient/images/reader-talk-sayit.png" border="0" /></a></div>';dojo.byId("chatInput").innerHTML="Type here.";this.rosterShowing=true;});dojo.subscribe('ubr/events/userLoggedIn',this,function(a){var time=a.time;this.init(a.user);});dojo.subscribe('ubr/events/userLoggedOut',this,function(a){dojo.byId("profileImg").getElementsByTagName('img')[0].setAttribute("src","");if(this.con){this.logout();this.con.disconnect();dojo.byId("chatLog").innerHTML='**You must be logged in to chat. ';}});this.UIinited=false;this.init=function(u){if(!u){console.warn('no user is defined, cannot init chat');return;}
